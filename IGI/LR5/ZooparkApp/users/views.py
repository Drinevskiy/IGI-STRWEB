...
def profile(request):
    if request.method == 'POST':
        ...
        count = request.POST.get('count-' + ticket_id)
    ...
    if not user.is_staff and not user.is_superuser:
        tickets_paid = Ticket.objects.filter(visitor = user, paid = True)
        tickets_not_paid = Ticket.objects.filter(visitor = user, paid = False)
        context["tickets_paid"] = tickets_paid 
        context["tickets_not_paid"] = tickets_not_paid 
        context["paid"] = tickets_not_paid.count() != 0
    if user.is_staff:
